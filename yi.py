      ...
      for square_ord in range(81):
        ...
        for d in digits:
          if d not in [self.get_value(p, node) for p in peers[square]]:
              yield from self.set_value(d, square, node)
              ...
              yield from self.set_value('.', square, node)
  ...
  def DFS(self):
      def search(board, square_ord, found  = False):
        ...
        if self.get_value(square, board) != '.':
          return (yield from search(board, square_ord+1))

        for d in digits:
          if d not in [self.get_value(p, board) for p in peers[square]]:
              yield from self.set_value(d, square, board)
              found = yield from search(board, square_ord+1)
              if found:
                return True
              yield from self.set_value('.', square, board)

      return (yield from search(self.board, 0))
...
#   solve_board(Solution.DFS, easy_board)
    print(f"Board is missing {str(hard_board).count('.')} values")
    listener = CountingLisener()
    solution = Solution(hard_board, listener)
    g = solution.DFS()
    # while(next(g)):
    #     pass
    for _ in g:
    ...
    print(f'Solved in {listener.no_calls} steps!\n\n')
